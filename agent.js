...
const History = [];
const ai = new GoogleGenAI({ apiKey: "YOUR_ACCESS_KEY" });
...
async function getWeatherInfo({city}){
    const response = await fetch(`http://api.weatherstack.com/current?access_key=${YOUR_ACCESS_KEY}&query=${city}`);
    const data = await response.json();
    return data;
}
...
const weatherInfoDeclaration = {
    ...
    parameters: {
        ...
        properties: {
            city: {
                type: 'STRING',
                description: 'It will be the city name, like New delhi, lucknow'
            },
        },
        required: ['city']
    }
...
async function runAgent(userProblem) {
    ...
    while (true) {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: History,
            config: {
                systemInstruction: `You are an AI Agent, You have access of 4 available tools like to
                        to find sum of 2 number, get crypto price of any currency, find a number is prime or not and get weather information for a city

                        Use these tools whenever required to confirm user query.
                        If user ask general question you can answer it directly if you don't need help of these four tools`,
                tools: [{
                    functionDeclarations: [sumDeclaration, primeDeclaration, cryptoDeclaration, weatherInfoDeclaration]
                }],
            },
        });
